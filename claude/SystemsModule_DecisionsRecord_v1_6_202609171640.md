...
**This version:** 17 September 2026, 16:40 AEST
**Supersedes:** `SystemsModule_DecisionsRecord_v1_5_202609171429.md`
...
- **v1.6 (17 Sep 2026, 16:40 AEST):** First security audit against the master working standard, Section 6. Five findings fixed, D29 to D33 recorded, and the project's first security section written. Typecheck reaches zero errors.
...
| D35 | 2026-09-17 | **No independent security review before go-live.** Julian, 17 Sep. Section 6 asks for one where the risk warrants it and asks that the decision be recorded either way; it is recorded here as declined. | The reasons to revisit, if any of them changes: the first real paying customer, the first venue that is not Julian's own, any handling of staff personal data beyond names and rosters, or a second operator's data sitting in the same tables with no row-level security behind the route checks. |
| D34 | 2026-09-17 | **A CI gate runs on every push to `develop` and `main`**, in `.github/workflows/ci.yml`: a Gitleaks secret scan over full history, `npm audit` blocking on high and critical, then typecheck, lint, test and build. | There was no CI at all. Netlify deploys from `develop` automatically, so until today the only gate between a push and production was Julian remembering to run the suite. |
| D29 | 2026-09-17 | **Security headers ship on every route**, set in `next.config.ts`: a Content-Security-Policy, HSTS at two years with preload, `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, a `Permissions-Policy` denying camera, microphone, geolocation and payment, `Cross-Origin-Opener-Policy`, and `poweredByHeader: false`. The CSP allow-lists only the origins the browser actually reaches: self, the Supabase project, and Sentry when a DSN is set. | The app shipped with none of these. Frameable, sniffable, no transport pinning, and any injected script could have called any origin. |
| D30 | 2026-09-17 | **`script-src` keeps `'unsafe-inline'`, deliberately and on the record.** | The App Router injects inline bootstrap and flight-data scripts on every page. Removing it needs a per-request nonce from middleware, which forces every page to render dynamically, and the public pages are static on purpose. Shipping a strong CSP with one named weakness beats shipping none while the perfect one is designed. The nonce upgrade is O12. |
| D31 | 2026-09-17 | **Uploads are checked against their bytes, not their label.** The declared MIME type was already allow-listed, but it comes from the browser and anyone calling the endpoint directly can set it freely. `sniffMimeType` now reads the leading bytes for PNG, JPEG, WEBP, PDF and the zip container behind .docx, and text is accepted only if it decodes as UTF-8 with no NUL byte in the first kilobyte. A mismatch and an unsupported type return the same message. | A shell script named `roster.png` announced itself as `image/png` and would have passed into storage and then into a model prompt. The messages match so the endpoint tells a prober nothing an honest operator does not also need. |
| D32 | 2026-09-17 | **The endpoints that spend money have hourly ceilings per venue:** 25 generations, 20 ingests, 40 audits. Enforced by counting rows already written, so it holds across serverless instances and needs no table and no migration. **It fails open** on a database fault. | Nothing throttled them before. Fails open because, unlike entitlement, this only decides whether an already-entitled operator has gone too fast, and refusing a paying venue over a hiccuped count is the worse failure. The ceilings sit above the whole nineteen-item catalogue so a real afternoon is never refused. |
| D33 | 2026-09-17 | **The seeded e2e subscription is stamped `metadata.synthetic = true`** at creation, and `scripts/flag-synthetic-subscriptions.ts` stamps any that predate the change. Every revenue or customer count must exclude `metadata->>'synthetic' = 'true'`. | That row says `active` and `group` and Stripe has never heard of it. It is currently the only row in the table. Stamping it is cheaper than remembering it. |
...
| O12 | 2026-09-17 | **Replace `'unsafe-inline'` in `script-src` with a per-request nonce** issued in middleware, accepting that the pages carrying it become dynamic. See D30. | Not blocking. The CSP is strong in every other directive. |
| O13 | 2026-09-17 | **The chat rate limiter is an in-memory `Map`** in `venue-advisor-chat.ts`, keyed by user and reset hourly. On serverless it resets whenever a new instance starts, so it enforces very little. The Systems endpoints now count rows instead; chat has no row to count, so it needs either a small table or a managed limiter. | Not blocking. Chat costs less per call than an audit. |
...
The project's first security record, written 17 September against Section 6 of the master working standard. Before this there was none, which is why several of the gaps below were omissions rather than decisions.

**The model.** Supabase Auth over `@supabase/ssr` for sessions. **No row-level security anywhere**, deliberately: ownership is enforced in every route through `resolveVenueAccess` and `requireSystemsContext`. This is an accepted risk, recorded rather than implicit. It means a single forgotten check in a single route is the whole failure, with nothing behind it. It is defensible at this size and should be revisited before the database holds more than a handful of venues.

**Authorisation.** Deny by default. Ownership is checked before plan, always, so a venue that is not yours returns 404 whatever you pay and no response reveals that a higher plan would have let you in (D27). Entitlement fails closed (D25). Every check is server-side; nothing relies on a hidden control.

**Secrets.** In Netlify's environment, per deploy, never in source and never shipped to the client. `.gitignore` covers `.env*` and the book manuscripts. Verified 17 September: no real secret in tracked source, the only matches being test fakes and prefix comparisons. Gitleaks now scans the full history on every push (D34 below).

**Payments.** Stripe holds the card data; this database holds only Stripe's identifiers. The webhook verifies its signature with `constructEvent` before acting on anything.

**Uploads.** Size, count, type allow-list and now a content check against the leading bytes (D31). Files land in a Supabase storage bucket under a path scoped to user and venue.

**What is accepted as-is, for now.** No WAF and no edge rate limiting in front of the app; the ceilings in D32 are application-level. No formal threat model per feature. No incident-response note beyond "rotate the key, revoke the sessions" living in this paragraph. Sentry is wired but only reports when a DSN is set.

**What is still open:** O12 the CSP nonce and O13 the chat limiter.

**Declined, on the record.** An independent security review before go-live (D35). Worth revisiting on the first real paying customer, the first venue that is not Julian's own, or the moment two operators' data sits in these tables with nothing but the route checks between them.
